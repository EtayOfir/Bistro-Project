 ...
 * <li>Delegating database operations to DAO classes (e.g.,
 * {@link ReservationDAO})</li>
 * <li>Returning responses to clients in a consistent response format</li>
 * </ul>
 * <p>
 * Database access is performed through a connection pool (HikariCP) initialized
 * via {@link mysqlConnection1#getDataSource()}, and accessed via
 * {@link ReservationDAO}.
 * <p>
 * The server also tracks connected clients and updates the GUI (if a UI
...
public class EchoServer extends AbstractServer {
	...
	/**
	 * DAO used to perform reservation-related DB operations (uses pooled
	 * connections).
	 */
	 ...
	 * <p>
	 * This method handles both removal by object reference and fallback removal by
	 * IP if the socket is already closed.
	 *
	...
	public void handleMessageFromClient(Object msg, ConnectionToClient client) {
		...
		try {
			...
			case "#CREATE_RESERVATION": {
				// Format: #CREATE_RESERVATION <numGuests> <yyyy-MM-dd> <HH:mm:ss>
				// <confirmationCode> <subscriberId> <phone> <email>
				if (parts.length < 6) {
			...
			case "#ADD_WAITING_LIST": {
				...
				// #ADD_WAITING_LIST <numDiners> <contactInfoB64Url> <confirmationCode>
				// [<subscriberId>]
				if (parts.length < 4) {
				...
				try {
					...
					if (parts.length >= 5) {
						try {
							subscriberId = Integer.parseInt(parts[4]);
						} catch (NumberFormatException ignored) {
						}
					}

					boolean inserted = waitingListDAO.insert(contactInfo, subscriberId, numDiners, confirmationCode,
							"Waiting");
			...
			case "#UPDATE_WAITING_ENTRY": {
				...
				// Format: #UPDATE_WAITING_ENTRY <waitingId> <numDiners> <contactInfoB64Url>
				// <status>
				if (parts.length < 5) {
				...
				try {
					...
					if (list == null || list.isEmpty()) {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							sb.append(r.getReservationId()).append(",").append(r.getCustomerType()).append(",")
			...
			case "#REGISTER": {
			    ...
			    if (parts.length < 2) {
			        ...
			        System.out.println("here 1" +msg.toString());
			        break;
			    }
			    if (subscriberDAO == null) {
			    ...
			    if (p.length < 7) {
			        ans = "REGISTER_ERROR|BAD_FORMAT";
			        System.out.println("here2"+msg.toString());
			        break;
			    }
			    ...
			    try {
			        // Unique username check (recommended)
	...
	/**
	 * Converts a {@link Reservation} into the wire protocol format returned to the
	 * client.
	 * <p>
	 * Format:
	 * {@code RESERVATION|<id>|<numGuests>|<date>|<time>|<confCode>|<subId>|<status>}
	 *
	 ...
	 * <p>
	 * This helper is used to avoid tight coupling between the server and the UI
	 * layer.
	 *
	...
	/**
	 * Called when the server begins listening for client connections. Initializes
	 * the DAO layer and prepares database access.
	 */
	@Override
	...
	/**
	 * Called when the server stops listening. Performs cleanup of clients and
	 * database pool.
	 */
	@Override
	...
	/**
	 * Called when a client successfully connects to the server. Adds the client to
	 * the tracking map and updates the UI.
	 *
	...
	/**
	 * Called when a client disconnects normally. Removes the client from the
	 * tracking map and updates the UI.
	 *
	 ...
	 * Expected token: SubscriberID=<number> (delimiter can be ;, |, whitespace or
	 * end) Returns null if not found or not parseable.
	 */
	...
	private String buildWaitingListProtocol() throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// New row format:
			// WaitingID,ContactInfoB64,NumOfDiners,ConfirmationCode,SubscriberID,Status,EntryTime
